...
namespace starrocks {
RollingAsyncOrcWriter::RollingAsyncOrcWriter(ORCInfo orc_info, const std::vector<ExprContext*>& output_expr_ctxs,
                              RuntimeProfile* parent_profile,
                              std::function<void(AsyncOrcChunkWriter*, RuntimeState*)> commit_func,
                              RuntimeState* state, int32_t driver_id) 
                        :  _orc_info(orc_info), _output_expr_ctxs(output_expr_ctxs), _parent_profile(parent_profile)
                        , _commit_func(commit_func), _state(state), _driver_id(driver_id) {
    _init();
};

Status RollingAsyncOrcWriter::_init() {
    ASSIGN_OR_RETURN(
            _fs, FileSystem::CreateUniqueFromString(_orc_info.partition_location, FSOptions(&_orc_info.cloud_conf)))
...
std::string RollingAsyncOrcWriter::_new_file_location() {
    _file_cnt += 1;
    _outfile_location = _partition_location + fmt::format("{}_{}_{}.orc", print_id(_state->fragment_instance_id()),
...
Status RollingAsyncOrcWriter::_new_chunk_writer() {
    ...
    _writer = std::make_shared<AsyncOrcChunkWriter>(
            std::move(writable_file), _options, _schema, _output_expr_ctxs,
